...
def get_dataloaders(dataset, imb_type, imb_factor, bs, num_workers, seed=42):
    random.seed(seed); np.random.seed(seed)
    # 선택 가능한 dataset: 'CIFAR100_LT', 'CIFAR10_LT'
    ...
    # dataset 불러오기
    base = ds_class('./data', train=True, download=True, transform=train_tf)
    test = ds_class('./data', train=False, download=True, transform=test_tf)

    # imbalanced subset 생성
